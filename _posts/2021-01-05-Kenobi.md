...
We'll start with scanning the host for open ports using [nmap](https://nmap.org/). The command we'll use is `sudo nmap -sV -T4 -p- -O -oN nmap kenobi` which is a full TCP-SYN scan to scan for open TCP ports on the target. Let's break it down:
...
`smbclient //kenobi/anonymous` (leave WORKGROUP empty). 
...
## SSH
Now we can use that private key to login as Kenobi using SSH - `ssh kenobi@kenobi -i id_rsa` where `-i` flag is used to specify the private key location.
